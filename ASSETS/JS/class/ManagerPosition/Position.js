...
export class ManagerPosition {
     ...
     */
    updateCoordinatesAuto(coordinatesPlayer, index) {
        ...
        if (xPlayer < x && xPlayer < x + this.speed) {
            x -= this.speed;
            this.utils.$("html").dispatchEvent(
                new CustomEvent("targetingPlayer", {
                    detail: {
        ...
        // else if (yPlayer > y && yPlayer > y + this.speed)
...
export class ManagerPositionWeapons {
    ...
    /**
     * UPDATE LES COORDS EN y des armes
     * @param {[x: number, y: number]} param0
     * @param {number} index
     */
    updateCoordinates([x, y], index) {
        ...
        /** @description { On set les nouvelles coordonnées des armes } */
    ...
    limiteArea([x, y], index) {
        if (y <= 0 || y >= this.canvas.canvasHeight) {
            this.utils.$("html").dispatchEvent(
